...
export default async function ArchivedFinalsPage() {
  const data = await loadArchivedTournamentData("doppio-2k26");
  const finalRanking = determineFinalRanking(data.matches, data.teams);
  return (
    <>
      <PageHero eyebrow="Archivio · Otto coppie, un titolo" title="Fase finale" description="Il tabellone definitivo del Doppio 2K26." logo logoSrc={data.tournament.logo_path} logoAlt={`Logo ${data.tournament.title}`} />
